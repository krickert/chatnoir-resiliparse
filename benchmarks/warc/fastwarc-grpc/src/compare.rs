...
fn main() {
    ...
    let l3 = l3_grpc_raw(&path, file_bytes, false);
    let l4 = l4_grpc_tonic(&path, file_bytes, false);
    println!();
    ...
    if let Some(gz_bytes) = gz {
        ...
        let g3 = l3_grpc_raw(&gz_path, gz_bytes, false);
        let g4 = l4_grpc_tonic(&gz_path, gz_bytes, false);
        gz_rows = Some((g0, g3, g4));
        println!();
    }
    ...
    ladder_row("L3 + gRPC transport (hand-framed)", l3.rate(), Some(l2.rate()));
    ladder_row("L4 + tonic codec (stock client/server)", l4.rate(), Some(l3.rate()));
    if let Some((g0, g3, g4)) = gz_rows {
...
/// Start a server on a fresh UDS; `raw` picks the codec-bypassing route.
async fn start_server(raw: bool) -> std::path::PathBuf {
    let sock = std::env::temp_dir().join(format!("fastwarc-compare-grpc-{}-{raw}.sock", std::process::id()));
    let _ = std::fs::remove_file(&sock);
...
/// Feed file chunks as `ParseWarcRequest`s; returns wire bytes it will put
/// on the wire (identical for the tonic codec and the hand-framed path,
/// since both produce the same encoding).
...
fn feed_raw(path: String, tx: tokio::sync::mpsc::Sender<Bytes>, config: pb::ParseWarcConfig) {
    std::thread::spawn(move || {
        let config = fastwarc_grpc::grpc_frame::encode_message(&pb::ParseWarcRequest {
            kind: Some(pb::parse_warc_request::Kind::Config(config)),
        });
...
fn upload_wire_bytes(file_bytes: u64, config: &pb::ParseWarcConfig) -> u64 {
    ...
    wire += full_chunks * (fastwarc_grpc::grpc_frame::chunk_prefix(CHUNK).len() as u64 + CHUNK as u64);
    let tail = file_bytes % CHUNK as u64;
    if tail > 0 {
        wire += fastwarc_grpc::grpc_frame::chunk_prefix(tail as usize).len() as u64 + tail;
    }
    wire
}
...
fn l3_grpc_raw(path: &str, file_bytes: u64, full: bool) -> Ledger {
    ...
    let ledger = measured(|| {
        runtime.block_on(async {
            let sock = start_server(true).await;
            let mut channel = connect(&sock).await;
            let (tx, rx) = tokio::sync::mpsc::channel::<Bytes>(512);
            feed_raw(path, tx, config);
            let request = http::Request::builder()
                .method(http::Method::POST)
                .uri("http://[::]:50051/fastwarc.v1.WarcService/ParseWarc")
            ...
            {
                use tower::Service;
                std::future::poll_fn(|cx| channel.poll_ready(cx)).await.unwrap();
            }
            let response = {
                use tower::Service;
                channel.call(request).await.unwrap()
            };
            ...
            while let Some(frame) = body.frame().await {
                ...
                wire_out += data.len() as u64;
                splitter.push(data);
                while let Some(message) = splitter
                    .next_message(fastwarc_grpc::transport::MAX_MESSAGE_SIZE)
                    .unwrap()
                {
                    let resp = pb::ParseWarcResponse::decode(message).unwrap();
                    count_end(&resp, &mut payload, &mut records);
                }
            }
            let _ = std::fs::remove_file(&sock);
    ...
    let (label, why) = if full {
        (
            "L3F hand-framed upload + full payload echo",
            "L3 with every payload byte streamed back: the wire carries the archive up AND down",
        )
    } else {
        (
            "L3  + gRPC transport (hand-framed messages)",
            "adds HTTP/2, flow control, and the async runtime over the socket; messages are hand-framed (11 B per 64 KiB chunk) and decoded by reference, so the codec copies are not in this row yet",
        )
    };
...
fn l4_grpc_tonic(path: &str, file_bytes: u64, full: bool) -> Ledger {
    let runtime = tokio::runtime::Runtime::new().unwrap();
    let config = parse_config(full, None);
    let wire_in = upload_wire_bytes(file_bytes, &config);
    let path = path.to_owned();
    ...
    let (label, why) = if full {
        (
            "L4F tonic codec upload + full payload echo",
            "L4 with every payload byte streamed back through the stock codec",
        )
    } else {
        (
            "L4  + tonic codec (stock client/server)",
            "what an off-the-shelf gRPC client pays: the codec copies each chunk into its frame buffer on send and re-merges frames on receive",
        )
    };
    print_row(label, why, &ledger);
    ledger
...
/// Request body streaming pre-framed gRPC messages (one DATA frame per
/// queued `Bytes`, no copy).
struct FrameBody {
