...
delivery, plus the HTTP/2 transport, plus the tonic codec. The cost of a
layer is the delta between adjacent rows, and each row prints both sides
of its ledger: bytes in, wire traffic in both directions, records out,
wall time, and CPU time. Run it before attributing a slowdown to any one
component:
...
|---|---:|---:|---|
| L0 parser, file on disk | 12496 | - | baseline: fastwarc + one disk-read copy |
| L1 + gRPC service (file stays on disk) | 12032 | -4% | gRPC session, protobuf metadata replies |
| L2 + socket delivery (no protocol) | 6897 | -43% | two kernel copies every socket consumer pays |
| L3 + gRPC transport (hand-framed) | 5939 | -14% | HTTP/2 framing, flow control, runtime hops |
| L4 + tonic codec (stock client/server) | 4567 | -23% | codec copies each chunk on send and receive |
| gzip: L0 parser, file on disk | 1493 | - | decompression-bound baseline |
| gzip: L4 tonic codec upload | 1448 | -3% | the whole gRPC stack, hidden behind gzip |

Two rows carry the story. L2 runs at the machine's raw socket ceiling
(7085 MiB/s here), which means the parser is fully overlapped and simply
waits on delivery: everything below L1 is the price of moving bytes
through a socket, not of gRPC and not of the parser. And the gzip rows
are within 3% of in-process parsing, because the wire moves compressed
bytes while the parser does identical work: for compressed archives the
transport tax effectively disappears.
